...
Function Copy-Folder([string]$source, [string]$destination, [bool]$recursive) {
   ...
   $source      = "https://subversion.acme.org:8080/!/#kmx/view/head/Architecture/IaC/Terraform/scripts/TF_modules"
   $destination = ".\"
   $recursive   =  $true

Copy-Folder -source $source -destination $destination -recursive $recursive